...
async fn circuit_connection_test() {
    ...
    caller.disconnect_circuit(session_id_cal.clone()).await;
    assert!(caller.get_circuit(session_id_cal.as_str()).is_none());

    std::thread::sleep(std::time::Duration::from_secs(1));
    assert!(answer.get_circuit(session_id_ans.as_str()).is_none());
}
...
pub async fn start_server(bootstrap_addr_option: Option<String>, port_option: Option<String>, key_type: String, key_pair: libp2p::identity::Keypair) -> Node {
    let mut node = whitenoisers::sdk::host::start(port_option, bootstrap_addr_option, whitenoisers::sdk::host::RunMode::Server, Some(key_pair), account::key_types::KeyType::from_str(key_type.as_str()));
